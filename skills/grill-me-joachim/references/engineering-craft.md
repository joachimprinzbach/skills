# Linse (c): Handwerk — Schnittstellen, Testing, Dokumentation, DevOps & Platform
...
Vier Bereiche. Kalibriere jeden am Pragmatismus-Regler (SKILL.md): ein Spike braucht keine Circuit Breaker, eine Plattform-Komponente schon. Zwei Dinge gelten aber **immer** als Blocker: **Secrets im Repo** und **gebrochene Contracts**.
...
- **Sicherheit (Basics, kein volles Threat-Model):** Input-Validierung, AuthN/AuthZ am Rand, Dependency-Hygiene (Scan auf bekannte Schwachstellen), Least Privilege, sichere Defaults, keine Secrets in Code/Logs. Offensichtliche Lücken (Injection, fehlende Autorisierung) sind Blocker.
...
*Review-Fragen:* Liegen Secrets im Repo? Ist Config aus dem Code heraus? Kann man den Dienst in Prod von außen debuggen? Haben I/O-Aufrufe Timeouts und sinnvolles Fehlerverhalten? Ist der Build reproduzierbar und der Deploy automatisiert?
