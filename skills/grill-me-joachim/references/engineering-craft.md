# Linse (c): Handwerk — Schnittstellen, Testing, Dokumentation, DevOps/Platform, VCS & KI
...
- **Test-first, wo es trägt (TDD).** Erst der rote Test, dann Code bis grün, dann refactor (**red-green-refactor**). Der Nutzen ist doppelt: Der Test *zuerst* zwingt zu einer nutzbaren API (du bist ihr erster Konsument) — das ist der direkte Hebel auf „Testbarkeit ist eine Design-Eigenschaft" oben — und liefert das Sicherheitsnetz fürs Refactoring. Tests als **Design-Druck**, nicht als Pflichtübung danach. Am Regler kalibriert: ein Wegwerf-Spike braucht kein TDD; ein Bugfix beginnt sinnvollerweise mit einem roten Test, der den Bug reproduziert. Kein Selbstzweck — Mittel für Design und Sicherheit (die Coverage-/Goodhart-Falle oben gilt weiter).
...
- **Build- & Dependency-Hygiene:** zentrale, konsistente Versionsverwaltung über Parent-POM/BOM (bzw. das Äquivalent im Ökosystem); **Plugin-/Dependency-Versionen gepinnt** — nichts „latest"/unversioniert, sonst ist der Build nicht reproduzierbar. **Keine SNAPSHOTs in Releases.** Dependency-Konvergenz und verbotene Dependencies via `maven-enforcer` (Werkzeug-Details in `clean-code.md`). Die Modul-Dependency-Struktur bleibt **zyklenfrei** und sauber geschnitten (Grenzen/Kohäsion: `architecture-and-domain.md`). Schwachstellen-/EOL-Scan siehe *Sicherheit* unten.
- **Infrastructure as Code (IaC):** Infrastruktur ist **deklarativ, versioniert und reviewt wie Code** (Terraform o. Ä.). **ClickOps** — Infrastruktur von Hand in der Cloud-Konsole zusammengeklickt — ist ein **klar benennbarer Fund**, kein Randdetail: weder reproduzierbar noch nachvollziehbar noch reviewbar. Auf IaC-Code gelten dieselben Standards wie auf App-Code: DRY (Module statt Copy-Paste), sprechende Namen, kein hartcodiertes Secret, `plan` vor `apply` als Review-Gate. **State** bewusst verwalten (Remote-State, Locking); Drift zwischen Code und Realität ist ein Fund. Umgebungen (dev/test/prod) aus *derselben* Quelle parametrisiert, nicht dreimal getrennt gepflegt.
...
## 6. KI-generierter Code & AI-Readiness
...
**AI-Readiness / Navigierbarkeit.** Dieselbe Struktur, die einen *Menschen* schnell navigieren lässt, macht die Codebase auch für KI-Agenten bearbeitbar — beides folgt aus gutem Schnitt, nicht aus KI-Magie. Prüf, ob die Codebase *navigierbar* ist: klar geschnittene Module mit sprechenden Namen (`architecture-and-domain.md`, `clean-code.md`), konsistente Struktur (ein Muster durchgezogen), maschinenlesbare Einstiegspunkte (README/Quickstart, ggf. `AGENTS.md`/`CLAUDE.md` mit Build/Test/Konventionen), typisierte Schnittstellen und Tests als ausführbare Spezifikation. Ein gut geschnittenes, konsistentes, dokumentiertes Repo ist per Definition AI-ready; ein verfilzter God-Service mit kryptischen Namen ist es für Mensch *und* Maschine nicht. AI-Readiness ist also kein Extra-Feature, sondern die sichtbare Belohnung für die Design-Disziplin der Linsen (a)/(b).

*Review-Fragen:* Wurde generierter Code verstanden und geprüft — oder nur durchgewinkt, weil er kompiliert? Fände sich ein Neuer (oder ein Agent) allein zurecht — klare Grenzen, sprechende Namen, maschinenlesbare Einstiegspunkte?
